...
class KBReference(Evaluatable):
    ...
    def __dict__(self) -> dict:
        return (
            dict(id=self.id, ref=self.ref.__dict__(), **(super().__dict__()))
            if self.ref is not None
            else dict(id=self.id, **(super().__dict__()))
        )
    ...
    def from_xml(xml: Element) -> "KBReference":
        return KBReference(
            xml.attrib["id"], KBReference.from_xml(xml.find("ref")) if xml.find("ref") is not None else None
        )
