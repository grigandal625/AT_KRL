...
class KBAllenOperation(KBOperation):
    ...
    def __init__(
        self,
        sign: str,
        left: str | KBEvent | KBInterval,
        right: str | KBEvent | KBInterval,
        events: List[KBEvent] | None = None,
        intervals: List[KBInterval] | None = None,
        rel_type: str = None,
    ):
        ...
        if isinstance(left, str) or isinstance(right, str):
            ...
            # Если неизвестная операция
                ...
                # если не вышло, пытаемся достать события и интервалы по ИД и определить тип операции
                    ...
                    self.get_tag_by_events_and_intervals(sign=sign, events=events, intervals=intervals)
            ...
            raise ReferenceError(
                "Expected types (str, str) or (KBEvent, KBEvent) or (KBInterval, KBInterval)"
    ...
    def get_tag_by_events_and_intervals(
        self, sign: str = None, events: List[KBEvent] | None = None, intervals: List[KBInterval] | None = None
    ):
        ...
        if events is not None and intervals is not None:
            ...
            if self._left in event_ids and self._right in event_ids:
                if self.for_what.get("event_event"):
                    ...
                    raise ValueError(f"Can not apply operation {self._left} {sign} {self._right} (event-event)")
                ...
                if self.for_what.get("event_interval"):
                    ...
                    raise ValueError(f"Can not apply operation {self._left} {sign} {self._right} (event-interval)")
